...
class BirthdayTable {
  render(dv: any) {
    ...
    // Get all people birth days

    // Create list of people with birth days within range exclude dead people <person.deceased = true>

    function birthdayList<Person extends GenericPerson>(people: Person[], range: {start: Date, end: Date}): Person[] {
      let birthdayList: Person[] = []
      people.forEach((person) => {
        if (person.deceased) {
          return
        } else if (person.birth > range.start && person.birth < range.end) {
          birthdayList.push(person)
        }
      })
      return birthdayList
    }
